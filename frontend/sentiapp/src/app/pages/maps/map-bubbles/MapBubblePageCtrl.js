(function () {
    ...
    /** @ngInject */
    function MapBubblePageCtrl(layoutColors, $timeout, layoutPaths, $scope, sentimentService) {
        var latlong = {};
        ...
        $scope.$on('countryEvent', function () {
            $scope.countrySelected = true;
        });

        var clickMapObject = function (event) {
            console.log(event)
            if (event.mapObject.objectType == "MapImage") {
                $scope.$broadcast('countryEvent', {
                    positive: event.mapObject.positive,
                    negative: event.mapObject.negative
                });
            }
        ...
        $scope.selectedCountries = [];
        map.addListener('clickMapObject', clickMapObject);
        ...
        $scope.search = function () {
            ...
            sentimentService.geoSearchSentiment(tags, $scope.selectedCountries)
                .then(function (success) {
                    var mapData = success.data;
                    ...
                    for (var i = 0; i < mapData.length; i++) {
                        var value = mapData[i].totalCount;
                        if (value < min) {
                            min = value;
                        }
                        if (value > max) {
                    ...
                    for (var i = 0; i < mapData.length; i++) {
                        ...
                        var value = dataItem.totalCount;
                        // calculate size of a bubble
                        var size = (value - min) / (max - min) * (maxBulletSize - minBulletSize) + minBulletSize;
                        if (size < minBulletSize) {
                            size = minBulletSize;
                        }
                        var negativeCount = dataItem.negative.length;
                        var positiveCount = dataItem.positive.length;
                        var neutralCount = dataItem.neutral.length;
                        var sentimentColor = layoutColors.primaryDark;
                        if (negativeCount > neutralCount) {
                            sentimentColor = layoutColors.danger;
                        }
                        if (positiveCount > neutralCount && positiveCount > negativeCount) {
                            sentimentColor = layoutColors.success;
                        }
                        if (positiveCount == negativeCount) {
                            sentimentColor = layoutColors.primaryDark;
                        }
                        dataProvider.images.push({
                            ...
                            longitude: dataItem.country.location.longitude,
                            latitude: dataItem.country.location.latitude,
                            title: dataItem.country.name,
                            value: value,
                            selectable: true,
                            //centered: false,
                            negative: dataItem.negative,
                            positive: dataItem.positive,
                            negativeCount: dataItem.negative.length,
                            positiveCount: dataItem.positive.length,
                            neutralCount: dataItem.neutral.length
                        });
                    }

                    var balloonText = '<span style="font-size:14px;"><b>[[title]]</b>: [[value]]</span>' +
                        '<br><span style="font-size:14px;"><b>Positive</b>: [[positiveCount]]</span>' +
                        '<br><span style="font-size:14px;"><b>Negative</b>: [[negativeCount]]</span>' +
                        '<br><span style="font-size:14px;"><b>Neutral</b>: [[neutralCount]]</span>';
                    map.dataProvider = dataProvider;

                    map.addListener('clickMapObject', clickMapObject);
                    map.balloonLabelFunction = function (obj, map) {
                        var currentText = balloonText;
                        if (obj.objectType == "MapImage") {
                            currentText = currentText.replace("[[title]]", obj.title);
                            currentText = currentText.replace("[[value]]", obj.value);
                            currentText = currentText.replace("[[positiveCount]]", obj.positiveCount);
                            currentText = currentText.replace("[[negativeCount]]", obj.negativeCount);
                            currentText = currentText.replace("[[neutralCount]]", obj.neutralCount);
                            return currentText;
                        }
                        return obj.title;
                    };
