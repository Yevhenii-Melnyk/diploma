...
import io.zengin.telegrambot.types.requests.ReplyMarkups.ReplyMarkup
...
trait Declarative {
  ...
  def reply(text: String, keyboard: Option[ReplyMarkup])(implicit message: Message) = {
    telegramApi.sendMessage(
      message.chat match {
        case Left(user) => SendMessageRequest(Right(user.id), text, replyTo = Some(message.id), replyMarkup = keyboard)
        case Right(groupChat) => SendMessageRequest(Left(groupChat.id), text, replyTo = Some(message.id))
      }
    )
  }
